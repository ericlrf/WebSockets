var table, VAZIA = -1, SIZE = 8, BRANCA = 0, PRETA = 1, origem = null, corDaVez = BRANCA;
...
function jogar() {
    var cor = corDaPeca(this);
    if (origem === null) {
        if (cor === corDaVez && this.firstChild !== null) {
        ...
        if (this.className === "selecionado") {
            limparPosicoes();
            if (origem !== this) {
                var tmp = origem.firstChild.cloneNode(true);
                origem.removeChild(origem.firstChild);
                if (this.firstChild)
                    this.removeChild(this.firstChild);
                this.appendChild(tmp);
                corDaVez = (corDaVez + 1) % 2;
//            alert("cor:"+corDaVez+"\nlinha:"+this.parentNode.rowIndex+"\ncoluna:"+this.cellIndex);
                send("cor:"+corDaVez+" linha:"+this.parentNode.rowIndex+" coluna:"+this.cellIndex);
            }
            origem = null;
        }
    }
...
//function drag(ev) {
//    var cor = corDaPeca(this.parentNode);
//    if (cor === corDaVez) {
...
//            this.innerHTML = data;
//            this.firstChild.ondragstart = drag;
